...
const DESIGN_THRESHOLD_MS = 10 * 60 * 1000; // 10 minutes
const CODING_THRESHOLD_MS = 20 * 60 * 1000; // 20 minutes
...
function Instructions() {
  return (
    <div className="text-sm text-tn-muted max-w-xl text-center mb-6">
      ...
      <p>
        <span className="text-tn-fg">Design phase:</span> 10 min target |{' '}
        <span className="text-tn-fg">Coding phase:</span> 20 min target
      </p>
    </div>
  );
}
...
export function App() {
  ...
  const designTimer = useTimer({ warningThresholdMs: DESIGN_THRESHOLD_MS });
  const codingTimer = useTimer({ warningThresholdMs: CODING_THRESHOLD_MS });
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6">
      <h1 className="text-2xl text-tn-fg mb-4">CodeKata</h1>

      {(phase === 'idle' || phase === 'ready') && (
        <div className="flex flex-col items-center gap-6 w-full">
          <Instructions />
          <ProblemInput onSelect={handleProblemSelect} />
          {phase === 'ready' && session.problem && (
            <button
